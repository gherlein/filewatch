...
/*----------------------------------------------------------------------*/
/* The contents below added by Greg Herlein May 2009 */
/*----------------------------------------------------------------------*/
int
MD5File(char* szFileName, char* szMD5)
{
  ...
  while(nContinue)
  {
    nLen=fread(szTemp,sizeof(char),4096,fpIn);
    if(nLen<4096)
    {
      /* probably need to check for error conditions here */
      if(feof(fpIn))
      {
        fclose(fpIn);
        nContinue=0;
